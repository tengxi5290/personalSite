<template>
    ...
    <!-- 水印 -->
      <el-checkbox v-model="params.mark" class="mark-box" v-if="isMark">水印</el-checkbox>{{index}}
    </div>
</template>
...
props:{
  ...
  methods: {
    ...
    postImage (filename) {
      ...
      console.log('组件里能拿到index')
      console.log(index)
      let param = new FormData();
      let fileUrl
      param.append('file',filename);
      
      let url = 'http://cms.cnlive.com:8768/upload/1'

      axiosProxyFiles.post(url, param).then(res => {
        console.log('图片传到自己的服务器上')
        console.log(res)
    ...
    changed (file, fileList) {
        ...
        console.log(file)
        // this.postImage(this.fileObj)
        // setTimeout (() => {
        //     console.log(this.currentUrl)
        //     this.$emit('change', file, this.currentUrl)
        // }, 0)
    },
    beforeUpload (files) {
      console.log(files)
    ...
    handleAvatarSuccess(res, file, fileList) {//上传成功服务器响应\
      console.log('success')
      console.log(res)
      this.$emit('get', res.data.url, this.index)
    },
