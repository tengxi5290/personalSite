        ...
        <div class="cms-content-left">
            <div class="tree-layout">
                <div class="tree">
                    ...
                    <el-tree ref="tree" :props="props" @check="check" :load="loadNode" lazy show-checkbox :indent='16' :node-key="props.id" :check-strictly="checkStrict" v-if="refresh">
...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                axiosProxy.get(this.$api.modelList).then(res => {
                    if(res.data.errorCode == '0') {
                        ...
                        for (let i in res.data.data) {
                            ...
                            obj.name = res.data.data[i].showName
            ...
            add () {
                let params = {}
                params.modelId = this.currentChannel.id
                params.modelName = this.currentChannel.name
                params.syncModel = this.dataInfo.name
                axiosProxy.post(this.$api.syncModel, params).then (res => {
                    if(res.data.errorCode == '0') {
                        this.successMessage('操作成功')
                        setTimeout (() => {
                            this.routerLink ('/sync/model', '', '')
                        }, 1000)
                    } else {
                        if(res.data.errorMessage) {
                            this.errorMessage(res.data.errorMessage)
            ...
            getUnableCahnnelList (callback) {
                axiosProxy.get(this.$api.syncModelList).then( res => {
                    if(res.data.errorCode == '0') {
                        for (let i in res.data.data) {
                            this.unableChannelList.push(res.data.data[i].modelId)
                        }
                        callback()
                    } else {
                        if(res.data.errorMessage) {
